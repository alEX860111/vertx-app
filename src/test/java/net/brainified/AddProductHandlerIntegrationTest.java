...
public class AddProductHandlerIntegrationTest extends IntegrationTest {
  ...
  public void testAddProduct(TestContext context) {
    ...
    when(dao.add(any(Product.class))).thenReturn(Single.just(product));

    final Async async = context.async();

    vertx.createHttpClient().post(HTTP_PORT, "localhost", "/products", response -> {
      ...
      response.handler(body -> {
        ...
        context.assertEquals("http://localhost:" + HTTP_PORT + "/products/" + resultProduct.getValue("_id"), response.getHeader("Location"));
        async.complete();
      });
    }).end(data.encode());
  }
  ...
  public void testAddProduct_serverError(TestContext context) {
    ...
    when(dao.add(any(Product.class))).thenReturn(Single.error(new RuntimeException("error")));
